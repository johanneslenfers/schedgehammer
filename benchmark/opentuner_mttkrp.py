...
ITERATIONS = 1000
REPETITIONS = 10
OUTPUT_DIR = "results_catbench/opentuner_mttkrp"
INVALID_COST = 1e12
...
class MttkrpOpenTuner(MeasurementInterface):
    ...
    def manipulator(self):
        ...
        m.add_parameter(EnumParameter('unroll_factor', exp_values_10))
        ...
        # OpenTuner PermutationParameter expects an iterable of items to permute
    ...
    def run(self, desired_result, input, limit=None):
        ...
        # Manual constraint: unroll_factor < chunk_size; penalize invalid
        if not (cfg['unroll_factor'] < cfg['chunk_size']):
...
def run_repetitions():
    print('=' * 60)
    print(f'Benchmarking: MTTKRP with OpenTuner')
    ...
    for rep in range(REPETITIONS):
        ...
        # Use AUCBandit-like approach: OpenTuner selects its own technique; keep defaults
        
        interface = MttkrpOpenTuner(args)
        # Run tuning with OpenTuner's main runner
        TuningRunMain(interface, args).main()
        
        # Save CSV (best only, see note in save_results_csv)
