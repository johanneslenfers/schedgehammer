...
class ScheduleTree:
    ...
    def _do_bfs(self, node_operation: Callable[[Node], bool]):
        """
...
class ScheduleParam(Param[Any]):
    ...
    current_population: list[tuple[ScheduleTree, float]] = field(
    ...
    def __post_init__(self):
        if self.use_genetic_algorithm_internally:
            for _ in range(self.population_size):
                ...
                cost = self.cost_function(schedule_tree)
    ...
    def create_random_schedule(self) -> ScheduleTree:
        while True:
            ...
            desired_length = random.randint(self.min_length, self.preferred_max_length)
            method_candidates = [
                method
                for method in self.api_description
                if method not in self.terminating_methods
    ...
    def choose_random(self, current_value=None):
        if self.use_genetic_algorithm_internally:
            ...
            population = sorted(self.current_population, key=lambda x: x[1])
