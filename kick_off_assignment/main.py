...
def cost(
        configuration: dict[str,
                            bool | float | int | str | list[int]]) -> float:
    ...
    # Real parameter
    term1: float = float(real_param * np.cos(np.sin(abs(real_param**2)))**2 -
                         0.5)  # type: ignore
    ...
    categorical_contrib: float = categorical_mapping.get(
        categorical_param, 0.0)
    ...
    performance_value: float = (switch_contrib + real_contrib +
                                integer_contrib + ordinal_contrib +
                                categorical_contrib + perm_contrib)
...
def main():
    config = Config({
        ...
        'power':
        OrdinalParameter([1, 2, 4, 8, 16]),
        'creature':
        CategoricalParameter([
            'dwarf', 'halfling', 'gold_golem', 'mage', 'naga', 'genie',
            'dragon_golem', 'titan'
        ]),
        'order':
        PermutationParameter([1, 2, 3, 4, 5])
    })
    tuner = Tuner(config)
    tuner.tune(cost)
